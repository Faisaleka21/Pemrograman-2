...
public class Query {
    ...
    public void simpan(String nama, String level, String varian, String harga){
    k.koneksiData();
    String sql="INSERT INTO `pembelian`(`nama`, `level`, `varian`, `harga`) VALUES (?,?,?,?)";
        
        try{
            PreparedStatement st = k.conn.prepareStatement(sql);
            st.setString(1, nama); //1= nilai kolom, nim= nilai dari inputan nim dst sampi 4
            st.setString(2, level);
            st.setString(3, varian);
